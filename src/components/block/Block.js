...
const Block = ({ previousHash, blockName, data, time, nonce, hash, mined, mine, dataInputChange }) => {
  return (
    <div className="card card-nav-tabs" style={{ marginTop: 50, padding: "auto 15" }}>
      { /*mined ?
        <div className="card-header card-header-success header-style">{ blockName }</div>
      :
        <div className="card-header card-header-warning header-style">{ blockName }</div>*/
      }
      <div className={`card-header header-style ${ mined ? 'card-header-success' : 'card-header-warning'}`}>{ blockName }</div>
      { mined ?
        <div className="card-body">
          <div>
            <div className="input-group" style={{ marginBottom: 10 }}>
              <span className="input-group-addon data-icon">
        ...
        <div className="card-body">
          <div>
            <div className="input-group">
              ...
              <DataInput mine={mine} data={data} dataInputChange={dataInputChange}/>
            </div>
            <button className="btn btn-warning button-style" onClick={mine}>Mine data</button>
          </div>
        </div>
      }
    </div>
